...
class AboneSerializer(serializers.ModelSerializer):
...
from .models import Ebulten

class EbultenSerializer(serializers.ModelSerializer):
    bulten = BultenlerSerializer(read_only=True)
    bulten_id = serializers.IntegerField(write_only=True, required=False)
    class Meta:
        model = Ebulten
        fields = ['id','baslik',"icerik",'bulten', 'bulten_id',"abonesayisi", 'is_removed']

    def create(self, validated_data):

        bulten_id = validated_data.pop('album_id', None)



        if bulten_id is not None:
            bulten = Bultenler.objects.get(id=bulten_id)
            validated_data['bultwn'] = bulten


        return Ebulten.objects.create(**validated_data)
