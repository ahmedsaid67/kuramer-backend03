...
class Ebulten(models.Model):
    baslik = models.CharField(max_length=200)
    icerik = models.TextField()
    bulten = models.ForeignKey(Bultenler, on_delete=models.CASCADE, null=True, blank=True)
    abonesayisi = models.IntegerField(default=0)
    is_removed = models.BooleanField(default=False)

    def save(self, *args, **kwargs):
        self.abonesayisi = Abone.objects.filter(durum=True, is_removed=False).count()

        super(Ebulten, self).save(*args, **kwargs)
